...
import { openbspEdgeRequest, openbspRestRequest, resolveOrganizationId } from './transport';

function parseJsonParam(value: unknown, fallback: IDataObject[] = []): IDataObject[] {
	if (Array.isArray(value)) return value as IDataObject[];
	if (typeof value === 'string') {
		const trimmed = value.trim();
		if (!trimmed) return fallback;
		const parsed = JSON.parse(trimmed);
		return Array.isArray(parsed) ? (parsed as IDataObject[]) : [parsed as IDataObject];
	}
...
export class OpenBsp implements INodeType {
	description: INodeTypeDescription = {
		...
		description: 'Send WhatsApp/Instagram messages and read data through OpenBSP',
...
async function handleMessage(
	...
	if (operation === 'getContext') {
		const conversationId = (this.getNodeParameter('conversationId', i, '') as string) || undefined;
		const account =
			(this.getNodeParameter('account', i, '', {
				extractValue: true,
			}) as string) || undefined;
	...
	switch (operation) {
		...
		case 'sendTemplate': {
			...
			const useRaw = this.getNodeParameter('useRawComponents', i, false) as boolean;
		...
		case 'sendLocation':
			...
			break;
		case 'sendContacts':
			content = contactsContent(parseJsonParam(this.getNodeParameter('contactsJson', i, '[]')));
			break;
		default:
...
async function handleTemplate(
	...
	const response = (await openbspEdgeRequest.call(this, 'PUT', 'whatsapp-management/templates', {
...
async function handleAccount(
	...
	const rows = await openbspRestRequest.call(this, 'GET', 'organizations_addresses', { qs });
	return toItems(rows);
}
