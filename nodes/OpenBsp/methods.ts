...
export async function getAccounts(
	...
	const rows = (await openbspRestRequest.call(this, 'GET', 'organizations_addresses', {
		qs: { select: 'address,status,extra', order: 'created_at.asc' },
	})) as AccountRow[];

	const results: INodeListSearchItems[] = rows
		.map((row) => {
			const label = row.extra?.verified_name ?? row.extra?.phone_number ?? row.address;
...
export async function getTemplates(
	...
	const templates: TemplateRow[] = Array.isArray(response) ? response : (response.data ?? []);
