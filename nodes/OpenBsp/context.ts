...
const CONVERSATION_SELECT = '*,organizations(*),contacts_addresses(*,contacts(*))';
...
export async function buildConversationContext(
	...
	if (opts.conversationId) {
		conversation = (await openbspRestRequest.call(this, 'GET', 'conversations', {
	...
	const { organizations, contacts_addresses, ...rest } = conversation as IDataObject & {
		organizations?: IDataObject;
		contacts_addresses?: IDataObject & { contacts?: IDataObject };
	};

	const contactAddress = (contacts_addresses as IDataObject | undefined) ?? null;
	const contact = (contacts_addresses?.contacts as IDataObject | undefined) ?? null;
