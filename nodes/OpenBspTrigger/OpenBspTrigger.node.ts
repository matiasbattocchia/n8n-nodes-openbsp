...
function mapEvent(
  event: string,
): { table: WebhookTable; operations: WebhookOperation[] } {
  switch (event) {
...
export class OpenBspTrigger implements INodeType {
  description: INodeTypeDescription = {
    ...
    description:
      "Starts the workflow on OpenBSP message or conversation events",
    ...
    properties: [
      {
        ...
        options: [
          ...
          },
          {
            ...
            description:
              "A message delivery status changed (sent, delivered, read, failed)",
          },
        ],
      },
      {
  ...
  webhookMethods = {
    default: {
      ...
      async create(this: IHookFunctions): Promise<boolean> {
        ...
        const created =
          (await openbspRestRequest.call(this, "POST", "webhooks", {
            body: {
  ...
  async webhook(this: IWebhookFunctions): Promise<IWebhookResponseData> {
    ...
    const serviceFilter = this.getNodeParameter(
      "serviceFilter",
      "any",
    ) as string;
    if (
      serviceFilter !== "any" && data.service && data.service !== serviceFilter
    ) {
      return { workflowData: undefined };
    }
